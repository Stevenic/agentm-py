...
def test_check_rate_limits():
    ...
    mock_cursor.fetchone.side_effect = [(100,), (50000,)]  # Return tuples for token usage and limit
...
def test_log_and_delete_api_usage():
    ...
    with mock.patch('sqlite3.connect', return_value=mock_conn):
        ...
        db.connect().cursor().execute("SELECT * FROM api_usage WHERE session_id = ?", ('session-1',))
        
        # Now simulate deleting the entry
        db.connect().cursor().execute("DELETE FROM api_usage WHERE session_id = ?", ('session-1',))
        
        # Assert the DELETE query was executed
        mock_cursor.execute.assert_any_call("DELETE FROM api_usage WHERE session_id = ?", ('session-1',))
